...
export interface Case {
  ...
  created_at?: string | Date; // Optional field for created date
}
...
const CaseDetail: React.FC<CaseDetailProps> = ({ caseId, onSelectCase, userInfo }) => {
  ...
  if (!userInfo || !caseId || !selectedCase) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center p-8 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl rounded-2xl border border-white/20 dark:border-slate-700/50 shadow-lg shadow-purple-500/5">
          <h3 className="text-lg font-bold bg-gradient-to-r from-violet-700 to-purple-700 dark:from-violet-300 dark:to-purple-300 bg-clip-text text-transparent mb-2">
                  ...
                  {isLoading ? (
                    ...
                    transactionData.map((transaction, index) => (
                      <tr
                        ...
                        className="border-b border-white/20 dark:border-slate-700/50 hover:bg-white/30 dark:hover:bg-slate-800/30 transition-colors">
                        ...
                        <td
                          className={`py-3 px-4 text-sm font-medium ${
                            transaction.amount && (transaction.amount.toString().startsWith("+") || transaction.amount > 0)
                          ...
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${
                              transaction.status === "Failed"
                                ? "bg-rose-100/70 text-rose-800 dark:bg-rose-900/50 dark:text-rose-300 border border-rose-200/50 dark:border-rose-800/50"
                                : transaction.status === "Disputed"
                                ? "bg-purple-100/70 text-purple-800 dark:bg-purple-900/50 dark:text-purple-300 border border-purple-200/50 dark:border-purple-800/50"
...
export default memo(CaseDetail, (prevProps, nextProps) => {
  ...
  return prevProps.caseId === nextProps.caseId && prevProps.userInfo === nextProps.userInfo;
});
