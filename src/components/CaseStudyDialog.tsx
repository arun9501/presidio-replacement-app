...
const CaseStudyDialog: React.FC<CaseStudyDialogProps> = ({ isOpen, onClose, selectedCase, userInfo }) => {
  ...
  useEffect(() => {
    if (isOpen && selectedCase) {
      ...
      // Try to fetch data from the new API endpoint first
      caseStudyService
        .getCaseSummary(selectedCase.id)
        .then((summaryData) => {
          // Convert the new API format to the existing format used by the UI
          const convertedData = caseStudyService.convertSummaryToAnalysis(summaryData);
        ...
        .catch((err) => {
          console.error("Failed to fetch case summary, falling back to generateCaseStudy:", err);

          // Fallback to the original method if the new API fails
          caseStudyService
            .generateCaseStudy(selectedCase)
            .then((data) => {
            ...
            .catch((fallbackErr) => {
              console.error("Failed to generate case study:", fallbackErr);
      ...
      <DialogContent className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border border-white/20 dark:border-slate-700/50 p-0 max-w-5xl w-[98vw] sm:w-[95vw] max-h-[95vh] md:max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader className="p-5 sm:p-7 border-b border-white/30 dark:border-slate-700/50 bg-gradient-to-r from-violet-50/50 to-purple-50/50 dark:from-violet-950/30 dark:to-purple-950/30 flex-shrink-0">
          ...
          <DialogDescription className="text-sm text-slate-600 dark:text-slate-400">
            Analyzing case #{selectedCase.id}: {selectedCase.title} for {userInfo.name}
          </DialogDescription>
        </DialogHeader>
                  ...
                  {analysisSteps.map((step, index) => {
                    ...
                    return (
                      <div
                        ...
                        {isActive && index < analysisStage && <CheckCircle className="h-4 w-4 sm:h-5 sm:w-5 text-emerald-500" />}
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="bg-white/70 dark:bg-slate-800/70 rounded-xl border border-white/30 dark:border-slate-700/50 p-3 sm:p-5">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-xs sm:text-sm">
                    ...
                    <span className="font-medium text-red-600 dark:text-red-400">Low ({analysisData.riskAssessment.deviceTrustScore}/100)</span>
                  </div>
                  <div className="flex justify-between text-xs sm:text-sm">
                    <span className="text-slate-600 dark:text-slate-400">Behavioral Match:</span>
                    <span className="font-medium text-red-600 dark:text-red-400">Poor ({analysisData.riskAssessment.behavioralMatch}/100)</span>
                  </div>
                  <div className="flex justify-between text-xs sm:text-sm">
              ...
              <div className="bg-white/70 dark:bg-slate-800/70 rounded-xl border border-white/30 dark:border-slate-700/50 p-3 sm:p-5">
                ...
                <div className="space-y-3">
                  {analysisData.recommendations.map((rec, index) => {
                    ...
                    const Icon = rec.type === "approve" ? CheckCircle : rec.type === "security" ? AlertCircle : FileText;
